...
class AllNovedadesGuardia {
  ...
  factory AllNovedadesGuardia.fromJson(String str) =>
      AllNovedadesGuardia.fromMap(json.decode(str));

  String toJson() => json.encode(toMap());

  factory AllNovedadesGuardia.fromMap(Map<String, dynamic> json) =>
      AllNovedadesGuardia(
        data: List<Result>.from(json["data"].map((x) => Result.fromMap(x))),
      );
...
class Result {
  ...
  factory Result.fromMap(Map<String, dynamic> json) => Result(
        ...
        nomObservacion: json["nomObservacion"],
        nomFotos:
            List<NomFoto>.from(json["nomFotos"].map((x) => NomFoto.fromMap(x))),
        nomVideo: List<dynamic>.from(json["nomVideo"].map((x) => x)),
        ...
        nomApelacionFotos:
            List<dynamic>.from(json["nomApelacionFotos"].map((x) => x)),
  ...
  Map<String, dynamic> toMap() => {
        ...
        "nomCiudad": nomCiudad,
        "nomFecha":
            "${nomFecha!.year.toString().padLeft(4, '0')}-${nomFecha!.month.toString().padLeft(2, '0')}-${nomFecha!.day.toString().padLeft(2, '0')}",
        "nomIdPer": nomIdPer,
        "nomDocuPer": nomDocuPer,
        "nomNombrePer": nomNombrePer,
        "nomApelacionTexto": nomApelacionTexto,
        "nomApelacionFotos":
            List<dynamic>.from(nomApelacionFotos!.map((x) => x)),
