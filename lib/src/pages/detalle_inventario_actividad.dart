...
class DetalleActividadInventarioInterno extends StatefulWidget {
  ...
  const DetalleActividadInventarioInterno(
      {super.key, required this.estado, required this.requeridos});

  @override
  State<DetalleActividadInventarioInterno> createState() =>
      _DetalleActividadInventarioInternoState();
}

class _DetalleActividadInventarioInternoState
    extends State<DetalleActividadInventarioInterno> {
  @override
  void initState() {
    super.initState();
    _inicio();
  ...
  Widget build(BuildContext context) {
    ...
    String fechaLocal = DateUtility.fechaLocalConvert(
        ctrl.getInfoActividad['act_asigFecReg']!.toString());
              ...
              child: Column(
                children: [
                  ...
                  SizedBox(
                    ...
                    // color: Colors.blue,
                    child: Text(
                        '${controller.getInfoActividad['act_asigEveNombre']} ',
                        style: GoogleFonts.lexendDeca(
                            // fontSize: size.iScreen(2.0),
                            fontWeight: FontWeight.normal,
                            color: Colors.grey)),
                      ...
                      ? Consumer<ActividadesAsignadasController>(
                          builder: (_, valueDS, __) {
                            return PaginatedDataTable(
                              ...
                              source: ListaInventarioInternoDTS(
                                  valueDS, widget.estado),
                            );
                          },
                        )
                          ...
                          builder: (_, valueMunicionesDS, __) {
                            return PaginatedDataTable(
                              ...
                              source: ListaInventarioInternoMunicionesDTS(
                                  valueMunicionesDS, widget.estado),
                            );
                          },
                        )
                      : const NoData(label: 'No hay registros '),
